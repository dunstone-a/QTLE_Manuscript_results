...
suppressPackageStartupMessages({
  ...
  devtools::load_all("/mnt/beegfs/mccarthy/scratch/general/adunstone/CBD_2021_qtlexperiment")
  devtools::load_all("/mnt/beegfs/mccarthy/scratch/general/adunstone/CBD_2021_multistateQTL")
})
...
For this study and to set default simulation parameters for `qtleParams()`, GTEx v8 eQTL summary statistics from 10 tissues with the largest sample size for eQTL mapping:
...
Data were downloaded from [Google Cloud](https://console.cloud.google.com/storage/browser/gtex-resources/GTEx_Analysis_v8_QTLs/GTEx_Analysis_v8_eQTL_all_associations?pageState=(%22StorageObjectListTable%22%3A(%22f%22%3A%22%255B%255D%22))&prefix&forceOnObjectsSortingFiltering=false&cloudshell=true)). To speed up calculations, only associations on chromosome 1 were used. The threshold for tests used to estimate significant parameters was <0.01 and for null QTL parameters was >0.1. 
...
if(rerun){
  ...
  system.time(gtex <- sumstats2qtle(input_hpc, 
                                    feature_id="gene_id",
  ...
  system.time(gtex <- getComplete(gtex, verbose=TRUE))
...
system.time(gtex <- callSignificance(gtex, 
                                     assay="pvalues",
                                     thresh=thresh, 
                                     second.thresh=second_thresh))
...
system.time(gtex_top <- runPairwiseSharing(gtex_top, 
                                           assay="significant", 
                                           factor=factor))

system.time(p_gtex <- plotPairwiseSharing(gtex_top, 
                                          cell_annotate = TRUE, 
                                          col_range = sharing))
...
p_qtl <- plotQTLClusters(gtex_top_multistate, FUN=abs, row_km = 5, 
                         annotate_tests = c("effect_sd", "nSignificant"))
draw(p_qtl)
...
params <- qtleEstimate(gtex, assay = "pvalues", verbose=TRUE,
                       thresh_sig=0.001, 
                       thresh_null=0.1)
