...
suppressPackageStartupMessages({
  ...
  library(cowplot)
  devtools::load_all("~/Scratch/CBD_2021_qtlexperiment/") # QTLExperiment
  devtools::load_all("~/Scratch/CBD_2021_multistateQTL/") # multiStateQTL
})
...
params <- multiStateQTL::qtleParams()
...
sim_simple <- multiStateQTL::qtleSimulate(params=params, ntests=n_tests, nstates=n_states, 
                           global=0.8, unique=0.2) 

sim_simple <- multiStateQTL::callSignificance(sim_simple, assay="lfsrs", 
                               thresh=thresh, second.thresh = second_thresh)
sim_simple <- multiStateQTL::runPairwiseSharing(sim_simple, assay="significant", factor=factor)
p_simple <- multiStateQTL::plotPairwiseSharing(sim_simple, col_range=sharing,
                                cell_annotate = TRUE)
draw(p_simple)
...
sim_sd <- multiStateQTL::qtleSimulate(params=params, ntests=n_tests, nstates=n_states, 
                       global=0.8, unique=0.2, 
                       beta_sd=c(0.1, 0.1, 0.1, 0.1, 1, 2))

sim_sd <- multiStateQTL::callSignificance(sim_sd, assay="lfsrs", 
                               thresh=thresh, second.thresh = second_thresh)
sim_sd <- multiStateQTL::runPairwiseSharing(sim_sd, assay="significant", factor=factor)
p_sd <- multiStateQTL::plotPairwiseSharing(sim_sd, col_range=sharing,
                                cell_annotate = TRUE)
draw(p_sd)
